...
import { TextField, Autocomplete, Button, Modal, Box, FormControl, InputLabel, Select, MenuItem, Checkbox, FormControlLabel, Typography } from '@mui/material';
...
function QuotaPrint() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [divisionsResponse, statusesResponse, sumUserResponse, yearResponse] = await Promise.all([
                    axios.get('http://localhost:5000/api/division', { params: { table: 'QuotaPrint' } }),
                    axios.get('http://localhost:5000/api/status'),
                    axios.get('http://localhost:5000/api/sumUser'),
                    axios.get('http://localhost:5000/api/year', { params: { table: 'QuotaPrint' } })
                ]);
    ...
    const fetchFilteredData = async (year, division, requester, startDate, endDate, status, blackWhite, color, deliveryDate) => {
        try {
            const [listResponse, sumYearResponse, sumUserResponse] = await Promise.all([
                ...
                axios.get('http://localhost:5000/api/sumUser', { params: { year } }),
            ]);
    ...
    const handleSearch = async (event) => {
        ...
        try {
            const response = await axios.get('http://localhost:5000/api/searchQuota', {
                params: {
                    searchText,
                    year: filterYear
                }
